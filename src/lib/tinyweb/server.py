...
class response:
    ...
    async def send_json(self, payload):
        """Send dict (or raw string) as JSON response.
        This function is generator.
        """
        ...
        self.add_header('Content-Length', str(len(res)))
        self.add_access_control_headers()
        await self._send_headers()
        await self.send(res)
...
async def restful_resource_handler(req, resp, param=None):
    ...
    # Call actual handler
    _handler, _kwargs, _async = req.params['_callmap'][req.method]
    # Collect garbage before / after handler execution
    gc.collect()
    if param:
        res = await _handler(data, param, **_kwargs) if _async else _handler(data, param, **_kwargs)
    else:
        res = await _handler(data, **_kwargs) if _async else _handler(data, **_kwargs)
    gc.collect()
...
class Webserver:
    ...
    def add_resource(self, cls, url, **kwargs):
        ...
        for m in ['GET', 'POST', 'PUT', 'PATCH', 'DELETE']:
            ...
            if hasattr(obj, fn):
                methods.append(m)
                callmap[m.encode()] = (getattr(obj, fn), kwargs, False)
                continue
            fn += "_async"
            if hasattr(obj, fn):
                methods.append(m)
                callmap[m.encode()] = (getattr(obj, fn), kwargs, True)
